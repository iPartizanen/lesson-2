class TimersManager {
  ...
  _startTimer(descriptor) {
    ...
    if (!timerId) {
      if (interval) {
        newTimerId = setInterval(() => this._log(descriptor, job(...callbackParams)), delay);
      } else {
        newTimerId = setTimeout(() => this._log(descriptor, job(...callbackParams)), delay);
      };
    };  
    return newTimerId;   
  }
...
manager.add(t1);
manager.add(t2, 1, 2).add(t3).add(t4, 22, 33);
manager.start();
manager.pause('t2');
manager.pause('t1');
manager.resume('t2');
