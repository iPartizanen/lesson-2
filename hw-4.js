  ...
  start() {
    if (!this.allStarted) {
      ...
      this.timers.forEach((descriptor) => { 
        ...
        if (descriptor.delay > maxTimeout) { maxTimeout = descriptor.delay };
      });
      // Реализовать общий таймаут для TimersManager. Время таймаута будет равняться
      //времени самого долгого таймера + 10 секунд.
      setTimeout(() => {
        this.timers.forEach((descriptor) => { 
          if (descriptor.timerId) { 
            descriptor.timerId.unref(); 
          };
        })
       }, maxTimeout + 10000);
    };  
